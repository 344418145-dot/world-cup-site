...
function renderDaySwitch(daily) {
  ...
  if (!switchEl) return;
  const options = [
    { mode: "tomorrow", label: "明日赛程", date: targetDateKey("tomorrow") },
    { mode: "today", label: "今日赛程", date: targetDateKey("today") }
  ];
  ...
  `).join("");
  switchEl.setAttribute("aria-label", `${formatDateLabel(daily.key)}赛程`);
}
